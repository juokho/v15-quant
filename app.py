...
# 1. 기본 설정 (폰트 충돌 방지를 위해 커스텀 CSS 제거)
    ...
    for i, t in enumerate(tickers):
        try:
            ...
            all_results.append({
                ...
                'LINK': f"https://toss.im/stock-info/S/{t}" # 토스 링크 데이터
            })
            if i % 20 == 0:
...
def display_leaderboard(df):
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "LINK": st.column_config.LinkColumn("LINK", display_text="🔗"), # 이모지로 깨짐 방지
            "Price": st.column_config.NumberColumn("Price", format="$ %.2f"), # Price 표기 고정
