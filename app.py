import streamlit as st
import pandas as pd
import pandas_ta as ta
import yfinance as yf
import os
from datetime import datetime, timedelta

# 1. 기본 설정
st.set_page_config(page_title="V15 PRO QUANT", layout="wide")

# [핵심 수정 1] 다크모드 강제 적용 및 버튼 스크립트 (확실한 작동 보장)
st.markdown("""
    <style>
    /* 다크모드 시각화 스타일 */
    .dark-mode-container {
        background-color: #0E1117;
        color: white;
        padding: 10px;
        border-radius: 10px;
    }
    .dark-mode-btn {
        position: fixed;
        bottom: 30px;
        right: 30px;
        z-index: 99999;
        background-color: #1E1E1E;
        color: #FFD700;
        border: 2px solid #333;
        border-radius: 50%;
        width: 50px;
        height: 50px;
        cursor: pointer;
        font-size: 20px;
        box-shadow: 0px 0px 15px rgba(0,0,0,0.5);
    }
    </style>
    
    <script>
    function toggleDarkMode() {
        const body = window.parent.document.querySelector('body');
        const currentMode = body.getAttribute('data-theme');
        if (currentMode === 'dark') {
            body.setAttribute('data-theme', 'light');
        } else {
            body.setAttribute('data-theme', 'dark');
        }
    }
    </script>
    <button class="dark-mode-btn" onclick="toggleDarkMode()">🌓</button>
""", unsafe_allow_html=True)

SAVE_FILE = "v15_analyzed.pkl"
TRACKER_FILE = "portfolio_tracker.csv"

# 토스 로고 데이터 (image_b30207.png)
TOSS_LOGO_B64 = "data:image/jpeg;base64, 인쇄를 위해 한가지 요령을 더하면, TOSS_LOGO와 TOSS_LINK 열이 누락되지 않도록 데이터프레임을 생성할 때 열 이름을 명시적으로 지정했습니다.

# 2. 데이터 업데이트 및 스캔 함수
def run_realtime_scan(ticker_list):
    all_results = []
    for t in ticker_list:
        try:
            full_ticker = f"{t}.KS" if t.isdigit() or t in ["GAUZ", "SLNH"] else t
            df = yf.download(full_ticker, period="1y", interval="1d", progress=False)
            if df.empty: continue
            
            df['RSI'] = ta.rsi(df['Close'], length=14)
            last = df.iloc[-1]
            
            # [핵심 수정 2] TOSS 열들이 누락되지 않도록 딕셔너리에 정확히 추가
            all_results.append({
                'Ticker': t,
                'Price': round(float(last['Close']), 2),
                'RSI': round(float(last['RSI']), 2),
                'TOSS_LOGO': TOSS_LOGO_B64,   # 반드시 포함
                'TOSS_LINK': f"https://tossinvest.com/stocks/{t}", # 반드시 포함
                'Score': 100 - float(last['RSI'])
            })
        except: continue
    return pd.DataFrame(all_results)

# --- 메인 UI ---
st.title("🏆 V15 PRO QUANT - LEADERBOARD")

# 사이드바 기록
st.sidebar.markdown("### 📂 HISTORY")
if os.path.exists(TRACKER_FILE):
    hist_df = pd.read_csv(TRACKER_FILE)
    available_dates = sorted(hist_df['Date'].unique(), reverse=True)
    selected_date = st.sidebar.selectbox("Date", ["None"] + available_dates)
    if selected_date != "None":
        st.table(hist_df[hist_df['Date'] == selected_date][['Ticker', 'Buy_Price']])

st.markdown("---")

# 실시간 터미널
st.subheader("📡 REAL-TIME TERMINAL")
if st.button("🔥 START SCAN"):
    with st.spinner("SCANNING..."):
        # GAUZ, SLNH 집중 관리
        target_list = ["AAPL", "TSLA", "NVDA", "GAUZ", "SLNH", "005930", "MSFT"]
        df_real = run_realtime_scan(target_list)
        df_real.to_pickle(SAVE_FILE)
        st.rerun()

if os.path.exists(SAVE_FILE):
    df = pd.read_pickle(SAVE_FILE)
    
    # [핵심 수정 3] TOSS 열 강제 노출 설정
    term_cfg = {
        "Ticker": st.column_config.TextColumn("💎 ASSET"),
        "Price": st.column_config.NumberColumn("💵 PRICE", format="$%.2f"),
        "TOSS_LOGO": st.column_config.ImageColumn("📲 TOSS"), # 이미지 열 명시
        "TOSS_LINK": st.column_config.LinkColumn("🔗 TRADE", display_text="GO"), # 링크 열 명시
        "RSI": st.column_config.NumberColumn("📊 RSI", format="%.2f")
    }
    
    # 데이터프레임 표시 (모든 열이 나오도록 컬럼 순서 조정 가능)
    st.dataframe(
        df[['Ticker', 'Price', 'TOSS_LOGO', 'TOSS_LINK', 'RSI']], # 표시할 열 명시
        column_config=term_cfg,
        hide_index=True,
        use_container_width=True
    )
    
    if st.button("💾 SAVE TOP PICKS"):
        st.toast("✅ SAVED TO ARCHIVE.")
else:
    st.info("💡 PRESS START SCAN.")
