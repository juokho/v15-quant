...
def run_full_market_scan():
    ...
    for i, t in enumerate(tickers):
        try:
            ...
            all_results.append({
                'Ticker': t,
                'Price_Val': float(last['Close']), # 포맷팅용 원본 숫자
                '거래대금_Val': int(last['Close'] * last['Volume']), # 포맷팅용 원본 숫자
                'Vol_Accel': float(last['Vol_Accel']),
                '반등점수': round(100 - float(last['RSI']), 1),
                '추세점수': round(float(last['MFI'] * last['Vol_Accel']), 1),
                'Toss': f"https://toss.im/stock-info/S/{t}" # 토스 링크
            })
            if i % 20 == 0:
...
def display_formatted_df(df, score_col):
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Price": st.column_config.TextColumn("Price"), # 미리 포맷팅된 문자열 출력
            "거래대금": st.column_config.TextColumn("거래대금"), # 미리 포맷팅된 문자열 출력
            "Vol_Accel": st.column_config.NumberColumn("거래가속", format="%.2f"),
            score_col: st.column_config.NumberColumn(score_col, format="%.1f"),
            "Toss": st.column_config.LinkColumn("Toss", display_text="🚀") # 예쁜 로켓 아이콘
        }
    )
...
if st.button("🔥 나스닥 실시간 스캔 시작 (V3.2)"):
