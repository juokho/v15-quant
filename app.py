...
st.markdown("""
    ...
    <button onclick="toggleDarkMode()" style="position:fixed;bottom:20px;right:20px;z-index:9999;border-radius:50%;width:50px;height:50px;cursor:pointer;">🌓</button>
...
if os.path.exists(SAVE_FILE):
    ...
    # [에러 해결] 필수 열이 없으면 에러 내지 말고 새로 생성
    for col in ['TOSS_LOGO', 'TOSS_LINK']:
        if col not in df.columns:
            df[col] = "" # 빈 값으로 일단 채움
            st.warning(f"⚠️ {col} 데이터가 누락되었습니다. 'START SCAN'을 다시 눌러주세요.")
    ...
    # 안전하게 열 선택 후 출력
    display_cols = ['Ticker', 'Price', 'TOSS_LOGO', 'TOSS_LINK', 'RSI']
    st.dataframe(df[display_cols], column_config=cfg, hide_index=True, use_container_width=True)
