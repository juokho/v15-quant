...
# --- 메인 UI ---
st.title("🛡️ V15 PRO - 나스닥 전종목 성적표 & 백테스트")
...
if st.button("🔥 나스닥 전체 실시간 스캔 시작 (Colab 과정 내재화)"):
    ...
    if not updated_df.empty:
        ...
        st.success(f"✅ 스캔 완료! (소요시간: {int(time.time() - start_time)}초)")
        st.rerun()

# 4. [추가] 과거 기록 표시 및 수익률 추적 구역
if selected_date != "선택 안 함":
    st.subheader(f"📅 {selected_date} 전략별 Pick & 수익률 추적")
...
if os.path.exists(SAVE_FILE):
    ...
    if st.button("💾 이 리스트를 오늘의 TOP으로 저장 (기록하기)"):
